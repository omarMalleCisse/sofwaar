...
export const ProduitsBranding = () => {
  ...
  return (
    <div className="min-h-screen bg-[#070620] py-1.5 px-4 sm:px-8 md:px-16 lg:px-32 xl:px-48">
      <h1 className="text-4xl font-bold mt-3.5 text-white mb-1 text-center drop-shadow-lg">Gestion des Brandings</h1>
      <p className="text-blue-100 text-center mb-2">Bienvenue sur la page de gestion des brandings.</p>
      ...
      <div className="flex flex-col items-center justify-center mt-6 bg-[#070620] text-blue-50">
        <TableGenerique
          ...
          columns={[
            { key: "id", label: "ID" },
            { key: "name", label: "Nom" },
            { key: "image", label: "Image", render: (img, row) => img ? <img src={"/uploads/"+img} alt={row.name} className="h-10 mx-auto" /> : null },
          ]}
          loading={loading}
          errors={errors && errors.toString()}
          onAdd={() => navigate('/add-branding')}
          addLabel="Ajouter un Branding"
          renderActions={branding => (
            <>
              <button
                ...
                onClick={() => navigate(`/edit-branding/${branding.id}`)}
              >
                Éditer
              </button>
              <button
                ...
                onClick={() => handleDeleteBranding(branding)}
              >
                Supprimer
              </button>
              <button
                ...
                onClick={() => navigate(`/branding/${branding.id}`)}
              >
                Détails
              </button>
            </>
...
export default ProduitsBranding;
