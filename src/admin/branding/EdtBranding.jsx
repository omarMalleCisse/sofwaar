...
const EdtBranding = () => {
  ...
  const brandingId = params.brandingId || params.id;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccess("Branding modifié avec succès !");
        setTimeout(() => {
          setSuccess("");
          navigate(-1);
        }, 1200);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#070620] py-16 px-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md bg-white/10 backdrop-blur-md rounded-2xl shadow-xl p-8 border border-blue-900">
        <h2 className="text-2xl font-bold text-white mb-6 text-center">Éditer un Branding</h2>
        ...
        <div className="mb-4">
          ...
          <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full px-3 py-2 rounded bg-blue-950/40 text-white border border-blue-900 focus:outline-none" placeholder="Nom du branding" />
        </div>
        <div className="mb-6">
