...
const AddBranding = () => {
  ...
  const { refetch } = useFetch("http://localhost:8000/branding/");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccess("Branding ajouté avec succès !");
        setTimeout(() => {
          setSuccess("");
          navigate(-1);
        }, 1200);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#070620] py-16 px-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md bg-white/10 backdrop-blur-md rounded-2xl shadow-xl p-8 border border-blue-900">
        <h2 className="text-2xl font-bold text-white mb-6 text-center">Ajouter un Branding</h2>
        ...
        <div className="mb-4">
          <label className="block text-blue-100 mb-2">Nom</label>
          <input
            type="text"
            value={name}
            onChange={e => setName(e.target.value)}
            className="w-full px-3 py-2 rounded bg-blue-950/40 text-white border border-blue-900 focus:outline-none"
            required
          />
        </div>
        <div className="mb-6">
        ...
        <button
          type="submit"
          className="w-full bg-blue-700 hover:bg-blue-800 text-white font-bold py-2 px-4 rounded-xl shadow transition-colors"
          disabled={loading}
        >
        ...
        <button
          type="button"
          className="w-full mt-3 bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-xl shadow transition-colors"
          onClick={() => navigate(-1)}
        >
          Annuler
        </button>
