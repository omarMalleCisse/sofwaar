...
const EdtFeatur = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccess("Feature modifiée avec succès !");
        setTimeout(() => {
          setSuccess("");
          navigate(-1);
        }, 1200);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#070620] py-16 px-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md bg-white/10 backdrop-blur-md rounded-2xl shadow-xl p-8 border border-blue-900">
        <h2 className="text-2xl font-bold text-white mb-6 text-center">Éditer une Feature</h2>
        ...
        <div className="mb-4">
          ...
          <input type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full px-3 py-2 rounded bg-blue-950/40 text-white border border-blue-900 focus:outline-none" placeholder="Titre de la feature" />
        </div>
        <div className="mb-4">
          <label className="block text-blue-100 mb-2">Texte</label>
          <textarea value={text} onChange={e => setText(e.target.value)} className="w-full px-3 py-2 rounded bg-blue-950/40 text-white border border-blue-900 focus:outline-none" placeholder="Texte de la feature" />
        </div>
        <div className="mb-6">
